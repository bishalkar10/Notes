...
// Type for routes that can be accessed with or without authentication
...
export const notesController = {
  ...
  async getNote(req: OptionalAuthRequest, res: Response) {
    const { id } = req.params;
    const note = await Note.findOne({ id }).select(
      "id user title content public createdAt updatedAt"
    );
    ...
    // - User can access their own notes (any visibility)
    // - User can access public notes from other users  
    // - User CANNOT access private notes from other users
    // - Unauthenticated users can only access public notes
  ...
  async updateNotePublicStatus(req: AuthenticatedRequest, res: Response) {
    ...
    const note = await Note.findOneAndUpdate(
      ...
      { public: isPublic },
      {
        new: true,
        runValidators: true,
        select: "id public createdAt updatedAt",
      }
    );
